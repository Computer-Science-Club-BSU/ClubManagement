...
def send_onboarding_email(token, user_obj):
    current_time = datetime.now()
    exp_dttm = current_time + timedelta(days=1)
    cfg = load_config()
    reset_address = cfg['public']['sys_addr']
    ...
    with open('src/interface/static/logo.png', 'rb') as f:
        logo_data = b64encode(f.read()).decode('utf-8')
    email_body = render_template('email_templates/onboarding_email.liquid',
                                 imgData=logo_data,
                                 resetLink=reset_link,
                                 recp=user_obj['email'],
                                 expDate=exp_date,
    ...
    send_email("Welcome to the Computer Science Club Management System",
               body=email_body,
               send_from="",
               to=[user_obj['email']],
               cc=[],
